...
namespace RBush
{
	public partial class RBush<T>
	{
		...
		#region ContainedByUtilities
		private List<T> FindCoveringBoxes(in Envelope boundingBox)
		{
			...
			while (queue.Count != 0)
			{
				...
				{
					foreach (T leafChildItem in item.children.Cast<T>())
						if (leafChildItem.Envelope.CoveredBy(boundingBox))
							containingNodes.Add(leafChildItem);
				}
				else
				{
					foreach (var child in item.children.Cast<Node>())
