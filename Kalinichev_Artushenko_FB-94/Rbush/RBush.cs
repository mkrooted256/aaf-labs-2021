...
namespace RBush
{
	public partial class RBush<T> : ISpatialDatabase<T>, ISpatialIndex<T> where T : ISpatialData
	{
		...
		public bool Contains(in Envelope boxToSearchFor)
		{
			var containingNodes = new List<T>();
			...
			while (queue.Count != 0)
			{
				...
				if (item.IsLeaf)
				{
					foreach (T leafChildItem in item.children.Cast<T>())
						if (leafChildItem.Envelope.Equals(boxToSearchFor))
							return true;
				}
				else
				{
					foreach (var child in item.children.Cast<Node>())
						if (child.Envelope.CoveredBy(boxToSearchFor))
							queue.Enqueue(child);
				}
			}
			return false;
		...
		public IReadOnlyList<T> SubsetOf(in Envelope boundingBox) =>
			FindCoveringBoxes(boundingBox);
